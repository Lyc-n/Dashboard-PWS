...
import { createFieldId, getNextOrder } from "@/lib/kunjungan-rumah-templates";
import type { KunjunganRumahSection, KunjunganRumahTemplateField, KunjunganRumahTemplates } from "@/lib/kunjungan-rumah-templates";
...
interface Props {
  ...
  setTemplates: React.Dispatch<React.SetStateAction<KunjunganRumahTemplates>>;
  resetTemplates: () => void;
  exportJson: () => void;
...
export function FormKunjunganRumahSection({ templates, setTemplates, resetTemplates, exportJson, importJson }: Props) {
  ...
  const getFields = (): KunjunganRumahTemplateField[] => {
  ...
  const setFieldsForSub = (next: KunjunganRumahTemplateField[]) => {
  ...
  const deleteField = (f: KunjunganRumahTemplateField) => {
  ...
  const openFieldDlg = (edit?: KunjunganRumahTemplateField) => {
    ...
    if (formSub === "sasaran") {
      ...
      const sectionMap: Record<string, KunjunganRumahSection> = {
  ...
  const saveFieldDlg = () => {
    ...
    const kind = fieldDlg.form.kind as KunjunganRumahTemplateField["kind"];
    ...
    if (fieldDlg.mode === "add") {
      ...
      const section: KunjunganRumahSection =
        formSub === "sasaran" ? (fieldDlg.form.sasaranSection as KunjunganRumahSection) : fieldDlg.section;
      const newField: KunjunganRumahTemplateField = {
        id,
        label,
        kind,
        section,
      ...
      const fields = currentFields.map((f) => {
        ...
        const section: KunjunganRumahSection =
          formSub === "sasaran" ? (fieldDlg.form.sasaranSection as KunjunganRumahSection) : f.section;
      ...
      >
        ...
        {formSub === "hasil" ? (
          <div className="mt-3 grid gap-2">
            <Toolbar>
              <span className="text-xs text-muted">{templates.hasilOpsi.length} opsi hasil kunjungan rumah</span>
              <Button
                size="sm"
                variant="primary"
                onClick={() => {
