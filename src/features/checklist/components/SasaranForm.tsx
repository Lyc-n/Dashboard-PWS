...
import type { PenilaianForm } from "@/hooks/use-kunjungan";
...
export function SasaranForm({ p, state, templates, dispatch }: Props) {
  ...
  const kolom = useMemo(() => fields.filter((f) => f.section === "sasaran:kolom" && !conditionalIds.has(f.id)), [fields, conditionalIds]);
      ...
      {bools.length > 0 ? (
        <div className="mt-3 grid gap-1.5">
          ...
          {rules
            ...
            .map((r) => {
              if (r.trigger === "periksaKehamilan") {
  
                // Render as per original grouping
                return (
                  <div key={r.trigger} className="grid gap-4 rounded-[8px] border border-line bg-muted/30 p-3">
