...
export function ChecklistFormScene({ record }: Props) {
  const { templates, state, dispatch, records, fillPercent, bahaCount, stepState, handleSubmit, reset, removeRecord, addFotos, setFotoCaption, removeFoto, fotoUploading } =
    useChecklistForm({ record });
  ...
  const handleDelete = (id: string) => {
    if (typeof window !== "undefined" && !window.confirm("Hapus kunjungan ini dari riwayat lokal? Tindakan tidak bisa dibatalkan.")) return;
    removeRecord(id);
    toast("Kunjungan dihapus dari riwayat.");
    if (editMode && id === record?.id) goCreate();
  };

  return (
    <AppShell>
      <PageHeader title={editMode ? "Edit Kunjungan" : "Input Checklist"} description="Checklist Kunjungan Rumah (KR) — form fleksibel diatur Admin di Kelola. Disimpan di perangkat ini." />
      ...
      {saved ? (
        editMode ? (
          <SuccessPanel title={`Perubahan kunjungan ${saved.info.namaKK || "keluarga"} tersimpan.`} message={`${saved.penilaian.length} penilaian sasaran · ${saved.masalah.length} masalah tercatat.`}>
            <Button variant="primary" onClick={goCreate}>Kembali & isi kunjungan baru</Button>
            <Link to="/checklist" className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-accent bg-accent px-4.5 py-2.75 text-[13px] font-bold text-on-accent hover:bg-accent-hover">Lihat Riwayat</Link>
          </SuccessPanel>
        ) : (
